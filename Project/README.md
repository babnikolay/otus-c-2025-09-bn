...
### В силу того, что не все процессоры (особенно старые или бюджетные) поддерживают инструкции AVX (Advanced Vector Extensions),  
### то оптимизация с использованием этих инструкций в коде не применялась.  
### При компиляции используется только опции -march=native -O3 для определения поддержки процессором инструкций AVX.  
...
```sh
$ ffmpeg -framerate 30 -i frames/frame_%g_%05d.png -c:v libx264 -pix_fmt yuv420p output.mp4
```
