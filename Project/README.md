...
### Для сборки всех программ необходимо запустить:
```
make
```
...
### 1. mandel.c
Базовый расчёт множества Мандельброта. Создаст файл в черно-белом изображении.
```
gcc -O3 mandel.c -o mandel -Wall -Wextra -Wpedantic
```

### 2. mandel_color.c
Тоже, что и предыдущий, но изображение в простой цветовой палитре (видны переходы между оттенками)
```
gcc -O3 mandel_color.c -o mandel_color -Wall -Wextra -Wpedantic
```

### 3. mandel_smooth.c
Тоже, что и предыдущий, но цветовая палитра сглажена (логарифмический алгоритм 
...
В Ubuntu для установки утилиты FFmpeg запустить:  
```
sudo apt install ffmpeg -y
```
Чтобы преобразовать полученные кадры в видео, выполнить в командной строке команду:
```
...
Стрелки перемещают изображение в стороны/вверх/вниз, левая клавиша "мыши" центрирует и приближает изображение, правая клавиша "мыши" центрирует и отдаляет изображение.  
```
