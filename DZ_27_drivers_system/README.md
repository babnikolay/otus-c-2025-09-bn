...
### Атрибут size позволяет пользователям настраивать размер буфера через sysfs, что уже предусмотрено в коде.

Настройка через sysfs
Для реализации настройки через sysfs в символьном устройстве нужно создать атрибуты, которые позволят 
пользователям взаимодействовать с модулем через файловую систему. В этом случае мы создадим атрибут для 
управления размером кольцевого буфера.

Шаги для добавления настройки через sysfs
Создайте атрибут в структуре kobject: Это позволяет определять параметры, которые будут доступны для чтения 
